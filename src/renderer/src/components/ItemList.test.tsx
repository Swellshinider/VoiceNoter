...
describe("ItemList", () => {
  it("shows empty state when no items", () => {
    render(<ItemList items={[]} selectedItemId={null} searchResults={[]} onSelectItem={vi.fn()} hasMore={false} />);
    expect(screen.getAllByText("No items yet.")[0]).toBeInTheDocument();
  });

  it("renders item rows with title and status badge", () => {
    render(<ItemList items={[mockItemSummary]} selectedItemId={null} searchResults={[]} onSelectItem={vi.fn()} hasMore={false} />);
  ...
  it("clicking an item calls onSelectItem with item ID", async () => {
    ...
    render(<ItemList items={[mockItemSummary]} selectedItemId={null} searchResults={[]} onSelectItem={onSelect} hasMore={false} />);
  ...
  it("shows search result snippets when provided", () => {
    render(<ItemList items={[{ ...mockItemSummary, ...mockSearchResult }]} selectedItemId={null} searchResults={[mockSearchResult]} onSelectItem={vi.fn()} hasMore={false} />);
  ...
  it("clicking a search result passes startSeconds", async () => {
    ...
    render(<ItemList items={[{ ...mockItemSummary, ...results[0] }]} selectedItemId={null} searchResults={results} onSelectItem={onSelect} hasMore={false} />);
    await user.click(screen.getAllByRole("button", { name: /Test Recording/ })[0]);
    expect(onSelect).toHaveBeenCalledWith("item-1", 42);
  });

  it("shows loading spinner when isLoading is true and no items", () => {
    render(<ItemList items={[]} selectedItemId={null} searchResults={[]} isLoading onSelectItem={vi.fn()} hasMore={false} />);
  ...
  it("shows load more when more items are available", async () => {
    ...
    render(<ItemList items={[mockItemSummary]} selectedItemId={null} searchResults={[]} onSelectItem={vi.fn()} hasMore onLoadMore={onLoadMore} />);
