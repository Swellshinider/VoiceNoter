...
import { mockJob, mockQueueSummary } from "./test-utils";

describe("QueueView", () => {
  it("shows empty state when no jobs", () => {
    render(<QueueView jobs={[]} summary={mockQueueSummary} onRetry={vi.fn()} onCancel={vi.fn()} hasMore={false} />);
  ...
  it("renders job rows with status badges", () => {
    ...
    render(<QueueView jobs={[job]} summary={mockQueueSummary} onRetry={vi.fn()} onCancel={vi.fn()} hasMore={false} />);
  ...
  it("retry button calls onRetry for failed jobs", async () => {
    ...
    render(<QueueView jobs={[job]} summary={mockQueueSummary} onRetry={onRetry} onCancel={vi.fn()} hasMore={false} />);
  ...
  it("cancel button calls onCancel for running jobs", async () => {
    ...
    const job = { ...mockJob, status: "running" as const };
    render(<QueueView jobs={[job]} summary={mockQueueSummary} onRetry={vi.fn()} onCancel={onCancel} hasMore={false} />);
  ...
  it("shows load more when more jobs are available", async () => {
    ...
    render(<QueueView jobs={[mockJob]} summary={mockQueueSummary} onRetry={vi.fn()} onCancel={vi.fn()} hasMore onLoadMore={onLoadMore} />);
